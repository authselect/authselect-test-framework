...
class SSHKeyUtils:
    ...
    def generate(
        ...
        :rtype: tuple[str, str]
        """
        # Only back up the specific home directory being touched, not all of
        # `/home`. Backing up `/home` itself is both unnecessarily broad and
        # unsafe: restoring it removes and recreates the `/home` directory
        # node (`rm -r /home`), which can intermittently fail with
        # "Device or resource busy" since `/home` is a shared system
        # directory that may be busy for reasons unrelated to this call.
        self.fs.backup(homedir)
