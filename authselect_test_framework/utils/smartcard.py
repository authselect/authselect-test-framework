...
class SmartCardUtils(MultihostUtility[MultihostHost]):
    ...
    def enroll_to_token(
        ...
        *,
        ...
        # p11_child verifies the smart card certificate's issuer against this CA
        # bundle. On a real "ipa-client-install"-enrolled host it is populated
        # automatically, but hosts enrolled by other means (e.g. via pre-provisioned
        # keytabs) never get it, so p11_child fails with "CA DB not found". Import
        # the IPA CA cert here so pre-auth certificate verification succeeds.
        ipa_ca_cert = ipa.fs.read("/etc/ipa/ca.crt")
        if not client.fs.exists("/etc/sssd/pki"):
            client.fs.mkdir_p("/etc/sssd/pki")
        client.fs.write("/etc/sssd/pki/sssd_auth_ca_db.pem", ipa_ca_cert, dedent=False)
