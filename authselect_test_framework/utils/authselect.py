...
def _profile_name(profile: Profile | str) -> str:
...
class AuthselectUtils(MultihostUtility[MultihostHost]):
    ...
    .. code-block:: python
        ...
        def test_example(client: Client, provider: GenericProvider):
            client.authselect.select(provider.profile, ['with-mkhomedir'])
    ...
    def select(self, profile: Profile | str, features: List[str] = []) -> None:
        """
        Select an authselect profile.

        :param profile: Authselect profile name or :class:`~authselect_test_framework.profiles.Profile`.
        :type profile: Profile | str
        :param features: Authselect features to enable, defaults to []
        :type features: list[str], optional
        """
        name = _profile_name(profile)
        self.host.conn.exec(["authselect", "select", name, *features, "--force", *self._backup_args()])
        self.assert_selected(profile, features)

    def assert_selected(self, profile: Profile | str, features: List[str] | None = None) -> None:
        """
    ...
    def is_feature_enabled(self, feature: str | List[str]) -> bool:
        ...
        Authselect does not provide an ``is-feature-enabled`` command on all
        supported versions, so this parses the feature list out of
        ``authselect current --raw`` instead (the current profile id followed
        by its enabled features). When multiple features are passed, returns
        ``True`` only if every feature is enabled.
        ...
        # First token is the profile id, the rest are the enabled features.
        enabled = set(self.current(raw=True).split()[1:])

        return all(name in enabled for name in features)
