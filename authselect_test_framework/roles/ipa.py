...
class IPAUser(IPAObject, GenericUser):
    ...
    def add(
        ...
        *,
        ...
        :param uid: User id, defaults to None (IPA auto-assigns from its configured ID range).
            A UID outside of that range gets no SID, and the KDC then fails the mandatory
            first kinit for that principal with "Generic error (see e-text)", so only pass
            an explicit value that is known to fall within the server's ID range.
        :type uid: int | None, optional
        :param gid: Primary group id, defaults to None (see ``uid``)
        ...
        if password and not require_password_reset:
            self._finalize_password(password)
    ...
    def _finalize_password(self, password: str) -> IPAUser:
        """
        Consume the mandatory "password must change at next login" flag.

        FreeIPA always marks a password as immediately expired whenever it is
        set or reset administratively (``user-add --password`` or ``ipa
        passwd``), see https://www.freeipa.org/page/New_Passwords_Expired.
        Setting ``password-expiration``/``krbPasswordExpiration`` does not
        clear this flag; only a genuine (self-service) password change does,
        even if the "new" password is identical to the current one. We
        perform that change here via a scripted ``kinit`` using a throwaway
        credential cache so the caller's active Kerberos ticket (typically
        the admin's) is left untouched, then push the expiration date far
        into the future so the password does not expire again during testing.

        :param password: Current (and resulting) password.
        :type password: str
        :return: Self.
        :rtype: IPAUser
        """
        principal = f"{self.name}@{self.role.realm}"
        ccache = f"/tmp/.ipa-pwfinalize-{uuid.uuid4().hex}"
        pwinput = f"{password}\n{password}\n{password}\n"

        try:
            self.role.host.conn.run(
                f"kinit {shlex.quote(principal)}",
                env={"KRB5CCNAME": ccache},
                input=pwinput,
            )
        finally:
            self.role.host.conn.run(f"rm -f {shlex.quote(ccache)}", raise_on_error=False)

        # add() already set password-expiration to the same far-future value, and the
        # self-service change above does not reset it, so this is normally a no-op that
        # IPA rejects with "no modifications to be performed". Only treat other failures
        # as errors.
        result = self.role.host.conn.run(
            f"ipa user-mod {shlex.quote(self.name)} --password-expiration 20380101120000Z",
            raise_on_error=False,
        )
        if result.rc != 0 and "no modifications to be performed" not in result.stderr:
            result.throw()
